...
func MainPage(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<html><body><H1>Welcome to Taylor's lab</H1> <div> Enjoy finding all the files </div></body>")
}
...
func FindFile(w http.ResponseWriter, r *http.Request) {
    ...
    if ok && len(q[0]) > 0 {
		...
		if !FOUND {
			jsonResp, _ := json.Marshal("no matches exist")
			w.Write(jsonResp)
		}
    } else {
...
func ResetArray(w http.ResponseWriter, r *http.Request) {
	log.Printf("Entering %s endpoint", r.URL.Path)
	w.Header().Set("Content-Type", "application/json")
	resetRegEx()
	Files = nil
} 
